import { Link, useNavigate } from "react-router";
import useAuthContext from "../Hooks/useAuthContext";
import { signOut } from "firebase/auth";
import { auth } from "../Firebase/firebase";

export default function Navbar() {
  const { username, userLoggedIn } = useAuthContext();
  const navigator = useNavigate();

  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[90%] bg-white/10 backdrop-blur-lg border border-white/20 shadow-lg rounded-3xl">
      <div className="navbar px-6 py-3">
        {/* Start / Hamburger for mobile */}
        <div className="navbar-start">
          <div className="dropdown">
            <div tabIndex={0} className="btn btn-ghost lg:hidden">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6 text-white"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M4 6h16M4 12h8m-8 6h16"
                />
              </svg>
            </div>
            <ul
              tabIndex={0}
              className="menu menu-sm dropdown-content bg-white/10 backdrop-blur-md rounded-box mt-3 w-52 p-2 shadow-lg text-white"
            >
              <li>
                <Link to="/">HOME</Link>
              </li>
              <li>
                <Link to="/movies">MOVIES</Link>
              </li>
              <li>
                <Link to="/about">ABOUT</Link>
              </li>
              <li>
                <Link to="/favourite">FAVORITES</Link>
              </li>
              {!userLoggedIn && (
                <>
                  <li>
                    <Link to="/login">LOGIN</Link>
                  </li>
                  <li>
                    <Link to="/signup">SIGNUP</Link>
                  </li>
                </>
              )}
            </ul>
          </div>

          <Link to="/" className="btn btn-ghost text-xl font-bold text-white">
            MoviePiX 🎬
          </Link>
        </div>

        {/* Center menu for larger screens */}
        <div className="navbar-center hidden lg:flex">
          <ul className="menu menu-horizontal px-1 text-white font-medium gap-4">
            <li>
              <Link to="/">HOME</Link>
            </li>
            <li>
              <Link to="/movies">MOVIES</Link>
            </li>
            <li>
              <Link to="/about">ABOUT</Link>
            </li>
            <li>
              <Link to="/favourite">FAVORITES</Link>
            </li>
            {!userLoggedIn && (
              <>
                <li>
                  <Link to="/login">LOGIN</Link>
                </li>
                <li>
                  <Link to="/signup">SIGNUP</Link>
                </li>
              </>
            )}
          </ul>
        </div>

        {/* End: Theme toggle + Avatar */}
        <div className="navbar-end flex items-center gap-4">
          {/* Theme Toggle */}
          {/* <label className="swap swap-rotate">
            <input type="checkbox" className="hidden" /> */}
          {/* Sun Icon */}
          {/* <svg
              className="swap-off w-10 h-10 fill-current text-yellow-400"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            >
              <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
            </svg> */}
          {/* Moon Icon */}
          {/* <svg
              className="swap-on w-10 h-10 fill-current text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            >
              <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
            </svg>
          </label> */}

          {userLoggedIn && (
            <h1 className="text-white font-medium">{`Hello ${username},`}</h1>
          )}
          {/* Avatar Dropdown */}
          {userLoggedIn && (
            <div className="dropdown dropdown-end">
              <div tabIndex={0} className="btn btn-ghost btn-circle avatar">
                <div className="w-10 rounded-full">
                  <img src="/userAvatar.png" alt="User Avatar" />
                </div>
              </div>

              <ul
                tabIndex={0}
                className="menu menu-sm dropdown-content bg-white/10 backdrop-blur-md rounded-box mt-3 w-52 p-2 shadow-lg text-white"
              >
                <li>
                  <a
                    onClick={() => {
                      signOut(auth);
                      navigator("/");
                    }}
                  >
                    Logout
                  </a>
                </li>
              </ul>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
